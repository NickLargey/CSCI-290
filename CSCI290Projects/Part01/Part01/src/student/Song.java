...
public class Song implements Comparable<Song> {
    ...
    public int compareTo(Song song2) {
        ...
        for (int i = 0; i < this.artist.length(); i++) {
            if (this.artist.charAt(i) < song2.artist.charAt(i)) {
                return BEFORE;
            }
            if (this.artist.charAt(i) > song2.artist.charAt(i)) {
                return AFTER;
...
//            System.out.println("In title char compare " + this.title.charAt(i) 
//                    + " and " + song2.title.charAt(i));
            if (this.title.charAt(i) < song2.title.charAt(i)) {
                return BEFORE;
            }
            if (this.title.charAt(i) > song2.title.charAt(i)) {
                return AFTER;
